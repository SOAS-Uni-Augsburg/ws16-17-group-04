turtles-own [infected energy immune reproduced-this-tick]
...
to setup-turtles
  ...
  ; determine the number of zombies that are initially created
  let number-of-immunes ((immune-percentage * (population-size - number-of-zombies)) / 100)
  ...
  ask turtles [
    ...
    ifelse (count turtles with [infected = true] < number-of-zombies) [
      ...
      set infected true
      set immune false
      set energy zombie-initial-energy
      set color red
    ]
    [
      ...
      set energy human-initial-energy
...
to go
  ...
  ask turtles [
    move
    reset-reproduce-flags
  ]


  ; zombies attack humans if there are any on the same patch
  ask turtles with [infected = true] [
    set energy energy - zombie-energy-decrease
    if(any? turtles-here with [infected = false]) [
      attack-human
      set energy energy + bite-energy-increase
    ]
  ]

  ask turtles with [infected = false] [
    if(any? turtles-here with [infected = false and reproduced-this-tick = false]) [
        reproduce
        set reproduced-this-tick true
    ]
  ]
...
to reproduce
  ask turtles-here with [infected = false and reproduced-this-tick = false ][
    set reproduced-this-tick true
    if (random 100 <= human-reproduce-probability) [
      ifelse (immune = true)
      [
        hatch 1 [
          set reproduced-this-tick true ; apparently newborns can't reproduce
          set energy human-initial-energy
          ifelse (random 100 < immune-percentage)
          [ set immune true
            set color yellow]
          [ set immune false
            set color grey ]
...
to reset-reproduce-flags
  set reproduced-this-tick false
end
...
"Immune Humans" 1.0 0 -4079321 true "" "plot count turtles with [immune = true]"
